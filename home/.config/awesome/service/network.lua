...
local string_trim = require("lib").string_trim
...
local function new()
	...
	ret._private.properties_proxy:connect_signal("PropertiesChanged", function(_, _, props)
		if props.NetworkingEnabled ~= nil then
			ret:emit_signal("network-state", props.NetworkingEnabled)
		end
		if props.WirelessEnabled ~= nil then
			ret:emit_signal("wireless-state", props.WirelessEnabled)
		end
	end)
	...
	gtimer.delayed_call(function()
		ret:emit_signal("network-state", ret:get_network_state())
		ret:emit_signal("wireless-state", ret:get_wireless_state())
	end)
