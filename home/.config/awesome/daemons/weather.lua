...
function weather_daemon:get_weather(out)
    local weather = {
        current = {},
        hourly = {},
        daily = {}
...
function weather_daemon:set_remote_watch()
    ...
    helpers.util.remote_watch(self.forecast_cmd, self.update_interval, self.tmp_file, function(out) self:get_weather(out) end)
end
