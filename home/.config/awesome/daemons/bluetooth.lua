...
local function get_device_info(self, object_path)
	if object_path ~= nil and object_path:match("/org/bluez/hci0/dev") then
		...
		if device_proxy.Name ~= "" and device_proxy.Name ~= nil then
			device_properties_proxy:connect_signal("PropertiesChanged", function(_, _, changed_properties)
			...
			gears.table.crush(device_proxy, device, true)
...
local function new()
	...
	gears.table.crush(ret, bluetooth, true)
	...
	if ret._private.object_manager_proxy then
		...
		ret._private.object_manager_proxy:connect_signal("InterfacesAdded", function(_, interface, data)
			get_device_info(ret, interface)
		end)

		ret._private.object_manager_proxy:connect_signal("InterfacesRemoved", function(_, interface, data)
			if interface ~= nil then
				ret._private.devices[interface] = nil
				ret:emit_signal(interface .. "_removed")
			end
		end)

		ret._private.adapter_proxy_properties:connect_signal("PropertiesChanged", function(_, interface, data)
