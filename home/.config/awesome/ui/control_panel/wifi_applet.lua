...
function wifi_applet:open_ap_menu(access_point)
	...
	self.connect_widget_obscure:buttons {
		awful.button({}, 1, function()
			obscure = not obscure
			self.connect_widget_text_input:set_obscure(obscure)
			if obscure then
				self.connect_widget_obscure.markup = text_icons.eye_off
			else
	...
	if access_point:is_active() then
		...
		self.connect_widget_text_input.exe_callback = function(input)
			access_point:connect(input, auto_connect)
		end
		self.connect_button:buttons {
			awful.button({}, 1, function()
				access_point:connect(self.connect_widget_text_input:get_input(), auto_connect)
				self:close_ap_menu()
			end)
		}
		self.connect_widget_text_input:run_keygrabber()
	end
end

function wifi_applet:close_ap_menu()
	if network_daemon:get_wireless_state() then
		self.connect_widget_text_input:stop_keygrabber()
		self.connect_widget_text_input:set_obscure(true)
		self.main_layout:reset()
...
function wifi_applet:on_scan_success(access_points)
	...
	for _, access_point in pairs(access_points) do
		...
		if access_point:is_active() then
			self.main_layout:insert(1, new_ap_widget)
		else
			self.main_layout:add(new_ap_widget)
		end
	end
end
...
function wifi_applet:on_wireless_state_changed(state)
	...
	if state then
		...
		self.control_button.fg = beautiful.bg
		self.control_button_sep:set_color(beautiful.bg)
		...
		self.control_button.bg = beautiful.bg_alt
		self.control_button.fg = beautiful.fg
		self.control_button_sep:set_color(beautiful.bg_urg)
...
local function new()
	...
	ret.control_button:connect_signal("mouse::enter", function(w)
		if not network_daemon._private.client_proxy.WirelessEnabled then
			w.bg = beautiful.bg_urg
			ret.control_button_sep:set_color(beautiful.fg_alt)
		end
	end)
	...
	ret.control_button:connect_signal("mouse::leave", function(w)
		if not network_daemon._private.client_proxy.WirelessEnabled then
			...
			ret.control_button_sep:set_color(beautiful.bg_urg)
		end
	end)
	...
	ret.connect_widget_text_input = widgets.text_input {
