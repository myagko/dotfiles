...
local notification_daemon = require("daemons.notifications")
...
function notification_list:create_notif(n)
	...
	local ntime = wibox.widget {
		widget = wibox.widget.textbox,
		markup = helpers.ui.set_markup(os.date("%H:%M"), { fg = beautiful.foreground_alt })
	}
...
local function new()
	...
	ret.dnd_button = helpers.ui.create_hover_button {
		markup = text_icons.bell_on,
		bg_normal = beautiful.background,
		margins = { right = dpi(16), left = dpi(16) },
		buttons = {
			awful.button({}, 1, function()
				notification_daemon:toggle_silent()
				if notification_daemon.silent then
	...
	ret.clear_button = helpers.ui.create_hover_button {
