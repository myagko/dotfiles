...
local function launch_app(app)
	local terminal_command = user.terminal and terminal_cmd[user.terminal] or nil
...
function launcher:filter()
	self.filtered = {}
	local filtered_any = {}
	local esc_input = helpers.lua_escape(self.text_input:get_input())
...
local function new()
	...
	ret.text_input = widgets.text_input {
	...
	local sidebar_home_button = widgets.hover_button {
	...
	local sidebar_wallpapers_button = widgets.hover_button {
	...
	local sidebar_poweroff_button = widgets.hover_button {
