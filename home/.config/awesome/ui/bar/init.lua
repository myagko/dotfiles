...
local function taglist(s)
	...
	local function t_callback(tw, t)
		...
		if t.selected then
			...
			t_selection.fg = beautiful.bg
		elseif #t:clients() > 0 then
			t_selection.bg = beautiful.bg_alt
			t_selection.fg = beautiful.fg
		else
			t_selection.bg = beautiful.bg_alt
			t_selection.fg = beautiful.fg_alt
		end
	...
	widget.widget_template.create_callback = function(tw, t)
		...
		tw:connect_signal("mouse::enter", function()
			if not t.selected then
				t_selection.bg = beautiful.bg_urg
			end
		end)
...
local function tasklist(s)
	...
	local function c_callback(tw, c)
		...
		c_container.bg = beautiful.bg_alt

		if c.minimized then
			c_container.fg = beautiful.fg_alt
		else
			c_container.fg = beautiful.fg
		end
