...
local function new()
	...
	audio_daemon:connect_signal("audio:vol_val", function(_, val)
		ret.vol_sli.value = tonumber(val/5)
		ret.vol_perc.markup = val .. "%"
	end)

	audio_daemon:connect_signal("audio:vol_mute", function(_, mute)
		if mute then
			ret.vol_sli.bar_active_color = beautiful.foreground_alt
			ret.vol_sli.handle_color = beautiful.foreground_alt
	...
	audio_daemon:connect_signal("audio:mic_val", function(_, val)
		ret.mic_sli.value = tonumber(val/5)
		ret.mic_perc.markup = val .. "%"
	end)

	audio_daemon:connect_signal("audio:mic_mute", function(_, mute)
		if mute then
			ret.mic_sli.bar_active_color = beautiful.foreground_alt
			ret.mic_sli.handle_color = beautiful.foreground_alt
